...
const Dashboard = () => {
  ...
  // 🟢 [नया एडिशन]: कंपनी नेम और पेजिनेशन के लिए स्टेट्स
  const [companyName, setCompanyName] = useState('SK Finance'); // डिफ़ॉल्ट नाम
  const [currentFilter, setCurrentFilter] = useState('All'); 
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 10; 

  const loadAllUsersForAdmin = async (token) => {
    try {
      const res = await axios.get('http://192.168.29.83:5000/api/auth/admin/users', {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const profileRes = await axios.get('http://192.168.29.83:5000/api/auth/me', { headers: { Authorization: 'Bearer ' + token } });
        setUserRole(profileRes.data.role);
        
        // अगर डेटाबेस में कंपनी का नाम पहले से सेव है तो वो उठाएगा
        if (profileRes.data.companyName) {
          setCompanyName(profileRes.data.companyName);
        }
        
        if (profileRes.data.role === 'super_admin') {
          loadAllUsersForAdmin(token);
        }

        const res = await axios.get('http://192.168.29.83:5000/api/clients/all', { headers: { Authorization: 'Bearer ' + token } });
  ...
  // 🟢 [नया एडिशन]: कंपनी का नाम चेंज करने का फंक्शन
  const handleEditCompanyName = async () => {
    const newName = prompt("Enter your Company/Business Name:", companyName);
    if (!newName) return;
    const token = localStorage.getItem('token');
    try {
      await axios.put('http://192.168.29.83:5000/api/auth/update-company', { companyName: newName }, {
        headers: { Authorization: 'Bearer ' + token }
      });
  ...
  const handleApproveAndPay = async (userId) => {
    try {
      const token = localStorage.getItem('token');
      await axios.put('http://192.168.29.83:5000/api/auth/admin/update-status/' + userId, {
        isApproved: true,
        paymentStatus: 'Paid'
      }, {
        headers: { Authorization: 'Bearer ' + token }
  ...
  const downloadPDF = (c) => {
    const doc = new jsPDF();
    doc.setFontSize(16);
    // 🟢 अब PDF में भी डायनामिक कंपनी का नाम प्रिंट होगा
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const token = localStorage.getItem('token');
    await axios.post('http://192.168.29.83:5000/api/clients/add', formData, { headers: { Authorization: 'Bearer ' + token } });
  ...
  const handleCollect = async (id) => {
    ...
    const token = localStorage.getItem('token');
    try {
      await axios.put('http://192.168.29.83:5000/api/clients/collect/' + id, { amountReceived: amount }, { headers: { Authorization: 'Bearer ' + token } });
      window.location.reload();
    } catch (err) {
  ...
  const handleAddPenalty = async (id) => {
    ...
    const token = localStorage.getItem('token');
    try {
      await axios.put('http://192.168.29.83:5000/api/clients/add-penalty/' + id, { amount, reason }, { headers: { Authorization: 'Bearer ' + token } });
      window.location.reload();
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    ...
    if (window.confirm("Are you sure you want to delete this client account?")) {
      const token = localStorage.getItem('token');
      try {
        await axios.delete('http://192.168.29.83:5000/api/clients/delete/' + id, { headers: { Authorization: 'Bearer ' + token } });
        window.location.reload();
      } catch (err) {
  ...
  const handleLogout = () => {
    if (window.confirm("Are you sure you want to logout?")) {
        localStorage.removeItem('token'); // टोकन डिलीट करेगा
        navigate('/login'); // वापस लॉगिन पेज पर भेज देगा
    }
};
  ...
  return (
   <div style={styles.container}>
      ...
      {/* 🟢 नया प्रोफेशनल हेडर - इसमें कंपनी का नाम और लॉग आउट बटन दोनों हैं */}
